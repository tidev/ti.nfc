...
@Kroll.proxy(creatableInModule = NfcModule.class,
			 propertyAccessors = { NfcConstants.PROPERTY_TITLE, NfcConstants.PROPERTY_URI, NfcConstants.PROPERTY_ACTION,
...
public class NdefRecordSmartPosterProxy extends NdefRecordProxy
{
	...
	public NdefRecord getRecord()
	{
		...
		if (hasProperty(NfcConstants.PROPERTY_ACTION)) {
			...
			records.add(
				new NdefRecord(NdefRecord.TNF_MIME_MEDIA, ACTION_RECORD_TYPE, new byte[0], new byte[] { action }));
		}
		if (hasProperty(NfcConstants.PROPERTY_MIME_TYPE)) {
			...
			records.add(new NdefRecord(NdefRecord.TNF_MIME_MEDIA, TYPE_TYPE, new byte[0],
									   mimeType.getBytes(Charset.forName("US_ASCII"))));
		}

		NdefMessage message = new NdefMessage(records.toArray(new NdefRecord[records.size()]));

		return new NdefRecord(NdefRecord.TNF_WELL_KNOWN, NdefRecord.RTD_SMART_POSTER, new byte[0],
							  message.toByteArray());
	}
				...
				for (NdefRecordProxy rec : records) {
					if (rec instanceof NdefRecordTextProxy) {
						...
						if (ACTION_RECORD_TYPE_STR.equals(rec.getType())) {
							...
							proxy.setProperty(NfcConstants.PROPERTY_MIME_TYPE,
											  new String(payload, Charset.forName("UTF-8")));
						}
					}
				}
			}
	...
	public static boolean isValid(NdefRecord record)
	{
		...
		return ((tnf == NdefRecord.TNF_WELL_KNOWN) && (Arrays.equals(record.getType(), NdefRecord.RTD_SMART_POSTER)));
	}
}
